...
CPoint CWallPic::GetCenterFromFrontWall(int x, int ebene, bool ofSecondStone) {
	...
	case 3:
		...
		if (x == 4) { xKoord = 36 * 2; yKoord = 25 * 2; }
		if (ofSecondStone) {
			yKoord -= 5 * 2;
		}
		break;
...
CPoint CWallPic::GetCenterFromSideWall(int x, int ebene, bool ofSecondStone) {
	...
	case 0:
		break;
	case 1:
		if (x == 2) { xKoord = 49 * 2; yKoord = 51 * 2; }  // Linke Schräge Mitte des großen Steins
		if (x == 3) { xKoord = 10 * 2; yKoord = 51 * 2; }  // Rechte Schräge Mitte des großen Steins
		if (ofSecondStone) {
			yKoord -= 14 * 2;
		}
		break;
	case 2:
		if (x == 2) { xKoord = 64 * 2; yKoord = 36 * 2; }  // Linke Schräge Mitte des großen Steins
		if (x == 3) { xKoord = 13 * 2; yKoord = 36 * 2; }  // Rechte Schräge Mitte des großen Steins
		if (ofSecondStone) {
			yKoord -= 11 * 2;
			xKoord += 3 * 2 * ((x == 2) ? 1 : -1);
		}
		break;
	case 3:
		if (x == 0) { xKoord = 29 * 2; yKoord = 25 * 2; }  // Entferne Schräge links 
		if (x == 1) { xKoord = 13 * 2; yKoord = 25 * 2; }
		if (x == 2) { xKoord = 72 * 2; yKoord = 25 * 2; }
		if (x == 3) { xKoord = 10 * 2; yKoord = 25 * 2; }
		if (ofSecondStone) {
			yKoord -= 8 * 2;
			xKoord += 3 * 2 * ((x == 2) ? 1 : (x==3) ? - 1 : 0);
		}
		break;
	}
	return CPoint(xKoord, yKoord);
...
CPoint CWallPic::GetBottomCenterFromSideWall(int x, int ebene) {
	...
	switch (ebene) {
	case 1:
		if (x == 2) { p.x = 49 * 2; p.y = 92 * 2; }  // Linke Schräge Mitte des großen Steins
		if (x == 3) { p.x = 10 * 2; p.y = 92 * 2; }  // Rechte Schräge Mitte des großen Steins
		break;
	case 2:
		if (x == 2) { p.x = 67 * 2; p.y = 64 * 2; }  // Linke Schräge Mitte des großen Steins
		if (x == 3) { p.x = 10 * 2; p.y = 64 * 2; }  // Rechte Schräge Mitte des großen Steins
		break;
	case 3:
		if (x == 0) { p.x = 30 * 2; p.y = 45 * 2; }  // Entfernte Schräge links 
