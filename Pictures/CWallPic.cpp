...
CPoint CWallPic::GetCenterFromSideWall(int x, int ebene, bool ofSecondStone) {
	...
	case 1:
		...
		break;
	case 2:
		if (x == 2) { xKoord = 67 * 2; yKoord = 33 * 2; }  // Linke Schräge Mitte des großen Steins
		if (x == 3) { xKoord = 10 * 2; yKoord = 33 * 2; }  // Rechte Schräge Mitte des großen Steins
		if (ofSecondStone) {
			yKoord -= 8 * 2;
		}
		break;
	case 3:
		if (x == 0) { xKoord = 29 * 2; yKoord = 22 * 2; }  // Entferne Schräge links 
