...
public:
	...
	VEKTOR GrpHeroWalkTo(VEKTOR to, boolean &collision);
	VEKTOR GrpMonsterWalkTo(VEKTOR fromPos, VEKTOR toPos, boolean &collision);
...
protected:
	...
	void DrawPressurePad(CDC* pDC, CDC* cdc, int xxx, int ebene, boolean isRound, bool isTiny);
...
private:
	...
	boolean TryToAttack(CGrpMonster* pGrpMon, COMPASS_DIRECTION direction, int range, int power, int absDist);
